...
function ChatCommands.SpammProtection( ply )
	...
	if last + ChatCommands.SpammDelay < CurTime() then
		...
		ply:PrintMessage( HUD_PRINTTALK, "Вы слишком часто используете эту функцию!" )
		return false
	end
end
...
function ChatCommands.DoChat( ply, str, itc )
	if string.sub(str,1,1) == "/" then
		...
		if ChatCommands.IsCommand( command ) then
			...
			ply:ChatPrint( "\""..command.."\" недопустимая команда!" )
		end
...
AddChatCommand( "w", "Разговаривать только с противоположным к вам игроком.", ChatCommands.Whisper )

function ChatCommands.Yell( ply, arg )
	local text = ply:Name() .." [Кричит]: " .. arg
	ChatCommands.PrintChatDistance( ply, 512, text )
end
AddChatCommand( "y", "Кричать!", ChatCommands.Yell, true )
...
AddChatCommand( "taunt", "Сыграть насмешку", ChatCommands.Taunt, true )
...
local CCMD_special = {}
CCMD_special["win"] = "sledbuild_classic/race/slb_win.mp3"
CCMD_special["lose"] = "sledbuild_classic/race/slb_lose.mp3"
CCMD_special["banana"] = "sledbuild_classic/taunts/slb_banana.mp3"
CCMD_special["freestyler"] = "sledbuild_classic/taunts/slb_freestyler.mp3"
CCMD_special["leeroy"] = "sledbuild_classic/taunts/slb_leeroy.mp3"
CCMD_special["speedboy"] = "sledbuild_classic/taunts/slb_speedboy.mp3"
