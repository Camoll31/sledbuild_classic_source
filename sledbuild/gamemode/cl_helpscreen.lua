...
function HUDDrawHelpScreen( startX, startY, bWidth )
	local Captions = { {"Команда",20}, {"Информация",80} }
	local bHeight = 0
	...
	surface.DrawText( "Помощь:"  )
	surface.SetTextPos( startX+16, startY+6 + RowHeight*1 )
	surface.DrawText( "Постройте сани, используя пропы. Приварите 'сваркой' сидения к своим саням."  )
	surface.SetTextPos( startX+16, startY+6 + RowHeight*2 )
	surface.DrawText( "затем перетащите свои сани в зону гонок и дождитесь начала."  )
	surface.SetTextPos( startX+16, startY+6 + RowHeight*3 )
	surface.DrawText( "Удачи " .. LocalPlayer():Nick() .. "!"  )
	surface.SetTextPos( startX+16, startY+6 + RowHeight*4 )
	surface.DrawText( "Список команд чата:"  )
