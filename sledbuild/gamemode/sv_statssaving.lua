local PLSpeedTable = {}
local PLAYERSTATS = "slb_players_v5" 
...
function stats_load( ply )
	...
	if ( result == nil ) then
		...
		ply:PrintMessage( HUD_PRINTTALK, "Добро пожаловать в Sledbuild Classic!, вся ваша статистика сохраняется!" )
	else
		for k, v in pairs( result ) do
			if ( v['mapname'] && v['topspeed'] && v['mapname'] == game.GetMap() ) then
				...
				if ( v['won'] && v['lost'] ) then
					ply:PrintMessage( HUD_PRINTTALK, "Всего выиграно гонок: "..v['won']..", Проиграно: "..v['lost'] )
				end
				break
			end
		end
...
function GenerateSubmitKey( ply ) 
...
function PST_CCSubmitSpeeds( ply, cmd, args )
	...
	if ply && ply:IsPlayer() && consolespam then
		Msg( "Игрок "..ply:Name().." не может обновиться ("..string.Implode("|",args)..")\n" )
	end
end
