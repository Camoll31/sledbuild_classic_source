...
resource.AddFile("materials/gui/sledbuild_classic/timer_bg.vtf")
resource.AddFile("materials/gui/sledbuild_classic/timer_bg.vmt")
resource.AddFile("materials/gui/sledbuild_classic/timer_ro.vtf")
resource.AddFile("materials/gui/sledbuild_classic/timer_ro.vmt")
resource.AddFile("materials/gui/sledbuild_classic/timer_sh.vtf")
resource.AddFile("materials/gui/sledbuild_classic/timer_sh.vmt")
resource.AddFile("materials/gui/sledbuild_classic/timer_bg2.vtf")
resource.AddFile("materials/gui/sledbuild_classic/timer_bg2.vmt")

resource.AddFile("sound/sledbuild_classic/race/slb_win.mp3")
resource.AddFile("sound/sledbuild_classic/race/slb_lose.mp3")
resource.AddFile("sound/sledbuild_classic/taunts/slb_banana.mp3")
resource.AddFile("sound/sledbuild_classic/taunts/slb_freestyler.mp3")
resource.AddFile("sound/sledbuild_classic/taunts/slb_leeroy.mp3")
resource.AddFile("sound/sledbuild_classic/taunts/slb_speedboy.mp3")
...
function NewRound()	
	...
	for k,v in pairs(ents.FindByClass("player")) do
		if ( v:Team() == TEAM_RACING && !v:InVehicle() ) then
			v:Kill()
		end
		if (v:InVehicle() && v:GetNWBool( "OnTrack" ) == true )then
			...
			if game.GetMap() == "slb_snowsled_v5" then
	...
	// Notify of new race
	for k,v in pairs(player.GetAll()) do
		v:PrintMessage( HUD_PRINTTALK , "Гонка [#" .. _G["HowMany"] .. "] началась.")
	end
...
function LimitVehicles( pl, mdl )
	if mdl != "models/vehicles/prisoner_pod_inner.mdl" && mdl != "models/nova/airboat_seat.mdl" then
		pl:PrintMessage( HUD_PRINTTALK, "Можно использовать только стандартное сиденье.")
		return false 
	end
end
...
function LimitTools( ply, trace, mode )
	if !ply:IsAdmin() and (trace.Entity:GetClass() == "func_brush" || trace.Entity:GetClass() == "player") then 
		ply:PrintMessage(HUD_PRINTTALK, "Этот проп не может быть удален.")
		return false
	end
end
...
function AdminNoclip( player, bool )
	...
	player:PrintMessage( HUD_PRINTTALK, "Только администраторы могут использовать 'noclip'.")
	return false
end
...
function LimitToolGuns( ply, trace, mode )
	...
	for k, v in pairs( slbrestools ) do
		if !ply:IsAdmin() and mode == v then
			ply:PrintMessage( HUD_PRINTTALK, "Этот инструмент заблокирован.")
			return false
		end
	end
end
...
function PlayarSpawnObject( ply )
	if ply:Team() == TEAM_RACING then
		ply:PrintMessage( HUD_PRINTTALK, "Пропы не могут быть заспавнены, пока вы находитесь в гоночной зоне.")
		return false
	end
	return true
end
...
function SpawnedProp( userid, model, prop )
  	if prop:BoundingRadius() > 128 then -- Note: Radius is half the size of the diameter (half the size of the prop)
   		prop:Remove()
		userid:PrintMessage( HUD_PRINTTALK, "Этот проп слишком большой для саней.")
	end
...
function blockProps( ply, mdl )
	for _, v in pairs( models ) do
		...
		ply:PrintMessage( HUD_PRINTTALK, "Этот проп заблокирован." )
   		return false
		end
	end
end
