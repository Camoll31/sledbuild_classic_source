-- slb_teleport trigger
	...
	function ENT:StartTouch( entity )
		...
		-- The player isn't supposed to get here without a vehicle.. kill him! >:-D
			if !game.GetMap() == "slb_snowsled_v5" then -- DON'T PUT TRIGGERS OVER NON-RACING AREAS!
				entity:Kill()
			end
		end
		...
		if entity:IsPlayer() and entity:InVehicle() then
			if self.AlreadyTeleported == 0 then
				...
				for k,v in pairs(player.GetAll()) do
					v:PrintMessage( HUD_PRINTTALK , "[1st] [" ..entity:Nick().. "] Первый!")
					v:PrintMessage( HUD_PRINTCENTER , "[1st] [" ..entity:Nick().. "] Первый!")
				end	
	...
	function ENT:TeleportEmptySled(entity)
		if entity:GetNWBool( "propisinteleport" ) then
			...
			for k,v in pairs(player.GetAll()) do
				v:PrintMessage( HUD_PRINTTALK , "[Sledbuild Classic] Потерянные вами сани заспавнены.")
			end	
		end
