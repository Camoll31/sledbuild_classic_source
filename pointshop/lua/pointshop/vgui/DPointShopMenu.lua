surface.CreateFont('PS_Heading', { font = 'GModNotify', size = 64 })
surface.CreateFont('PS_Heading2', { font = 'GModNotify', size = 24 })
surface.CreateFont('PS_Heading3', { font = 'GModNotify', size = 19 })

surface.CreateFont( "PS_Default", {
	font = system.IsLinux() and "GModNotify" or "GModNotify",
...
local BGColor1 = Color(69, 69, 69)
local BGColor2 = Color(117, 117, 117)
local BGColor3 = Color(117, 117, 117)
...
function PANEL:Init()
	...
	if (PS.Config.AdminCanAccessAdminTab and LocalPlayer():IsAdmin()) or (PS.Config.SuperAdminCanAccessAdminTab and LocalPlayer():IsSuperAdmin()) then
		...
		ClientsList:AddColumn('Предметы'):SetFixedWidth(60)
		...
		ClientsList.OnClickLine = function(parent, line, selected)
			...
			menu:AddOption('Установить '..PS.Config.PointsName..'...', function()
				Derma_StringRequest(
					"Установить "..PS.Config.PointsName.."" .. ply:GetName(),
					"Установить "..PS.Config.PointsName.." в...",
					"",
					function(str)
			...
			menu:AddOption('Передать '..PS.Config.PointsName..'...', function()
				Derma_StringRequest(
					"Передать "..PS.Config.PointsName.."" .. ply:GetName(),
					"Передать "..PS.Config.PointsName.."...",
					"",
					function(str)
						if not str or not tonumber(str) then return end
			...
			menu:AddOption('Взять '..PS.Config.PointsName..'...', function()
				Derma_StringRequest(
					"Взять "..PS.Config.PointsName.." из " .. ply:GetName(),
					"Взять "..PS.Config.PointsName.."...",
					"",
					function(str)
						if not str or not tonumber(str) then return end
			...
			BuildItemMenu(menu:AddSubMenu('Взять предмет'), ply, OWNED_ITEMS, function(item_id)
...
function PANEL:Paint(w, h)
	...
	if PS.Config.CommunityName then
		draw.SimpleText(PS.Config.CommunityName .. " ", 'PS_LargeTitle', 16, 8, color_white)
	else
		draw.SimpleText("", 'PS_LargeTitle', 16, 8, color_white)
	end

	draw.SimpleText('У вас есть ' .. LocalPlayer():PS_GetPoints() .. ' ' .. PS.Config.PointsName, 'PS_Heading3', self:GetWide() - 40, 24, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
