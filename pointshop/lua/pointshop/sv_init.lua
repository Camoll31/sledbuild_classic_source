...
net.Receive('PS_SendPoints', function(length, ply)
	...
	if not ply:PS_HasPoints(points) then
		ply:PS_Notify("Вы не можете позволить себе отдать ", points, " свою ", PS.Config.PointsName, ".")
		return
	end

	ply.PS_LastGavePoints = ply.PS_LastGavePoints or 0
	if ply.PS_LastGavePoints + 5 > CurTime() then
		ply:PS_Notify("Стоять! Нельзя так быстро передавать очки.")
		return
	end

	ply:PS_TakePoints(points)
	ply:PS_Notify("Вы дали ", other:Nick(), " ", points, " из ваших ", PS.Config.PointsName, ".")
...
function PS:LoadDataProvider()
	...
	if not file.Exists(path, "LUA") then
		error("Поставщик данных Pointshop не найден. " .. path)
	end
