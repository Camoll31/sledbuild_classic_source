...
function Player:PS_PlayerInitialSpawn()
	...
	if PS.Config.NotifyOnJoin then
		...
		if PS.Config.ShopChatCommand ~= '' then
			timer.Simple(5, function() 
				if !IsValid(self) then return end
				self:PS_Notify('Введите ' .. PS.Config.ShopChatCommand .. ' в чат, чтобы открыть Игровой Магазин!')
			end)
		end
	...
	if PS.Config.CheckVersion and PS.BuildOutdated and self:IsAdmin() then
		timer.Simple(5, function()
			if !IsValid(self) then return end
			self:PS_Notify("PointShop устарел, сообщите об этом владельцу сервера!")
		end)
	end
	...
	if PS.Config.PointsOverTime then
		timer.Create('PS_PointsOverTime_' .. self:UniqueID(), PS.Config.PointsOverTimeDelay * 60, 0, function()
			...
			self:PS_Notify("Вам дали ", PS.Config.PointsOverTimeAmount, " ", PS.Config.PointsName, " за игру на сервере!")
		end)
	end
end
...
function Player:PS_CanPerformAction(itemname)
	...
	if not allowed then
		self:PS_Notify('Вам не разрешено делать это в данный момент!')
	end
...
function Player:PS_BuyItem(item_id)
	...
	if ITEM.AdminOnly and not self:IsAdmin() then
		self:PS_Notify('Этот пункт только для администратора!')
		return false
	end
	...
	if ITEM.AllowedUserGroups and #ITEM.AllowedUserGroups > 0 then
		if not table.HasValue(ITEM.AllowedUserGroups, self:PS_GetUsergroup()) then
			self:PS_Notify('Вы не в той группе, чтобы купить этот товар!')
			return false
		end
	end
	...
	if CATEGORY.CanPlayerSee then
		if not CATEGORY:CanPlayerSee(self) then
			self:PS_Notify('Вам не разрешено покупать этот предмет!')
			return false
		end
	end
	...
	if ITEM.CanPlayerBuy then 
		...
		if not allowed then
			self:PS_Notify(message or 'Вам не разрешено покупать этот предмет!')
			return false
		end
	end
	...
	if ITEM.SingleUse then
		self:PS_Notify('Предмет одноразового использования. В следующий раз вам придется снова купить этот товар!')
		return
	end
...
function Player:PS_SellItem(item_id)
	...
	if ITEM.CanPlayerSell then 
		...
		if not allowed then
			self:PS_Notify(message or 'Вам не разрешено продавать этот предмет!')
			return false
		end
	end
...
function Player:PS_EquipItem(item_id)
	...
	if not allowed then
		self:PS_Notify(message or 'Вам не разрешено надевать этот предмет!')
		return false
	end
...
function Player:PS_HolsterItem(item_id)
	...
	if not allowed then
		self:PS_Notify(message or 'Вам не разрешено носить этот предмет в кобуре!')
		return false
	end

	ITEM:OnHolster(self)

	self:PS_Notify('Кобура ', ITEM.Name, '.')
